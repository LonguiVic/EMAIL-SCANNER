...
colors = {
    ...
    'tree_odd_row': '#F3F4F9',             # Cor de fundo das linhas ímpares da tabela
    'tree_even_row': '#E7E8ED'             # Cor de fundo das linhas pares da tabela
}

root = tk.Tk()
root.title('Verificador de E-mail')
root.geometry('419x210')
root.configure(bg=colors['background'])

frame_main = tk.Frame(root, width=419, height=60, bg=colors['background'], pady=0, padx=0, relief="flat")
frame_main.grid(row=0, column=0)

frame_tabela = tk.Frame(root, width=405, height=110, bg=colors['background'], pady=0, padx=0, relief="flat")
frame_tabela.grid(row=1, column=0)

style = ttk.Style(frame_main)
style.theme_use("clam")

lista_valid_email = []   # Lista para armazenar os emails válidos
lista_invalid_email = [] # Lista para armazenar os emails inválidos
...
def abrir_ficheiro():
    ...
    emails = []  # Inicializando a lista de emails

    with open(ficheiro) as file:
        ler_csv = csv.reader(file)
        for row in ler_csv:
            verificador_de_e_mail(row[2])   # Verifica o email na coluna 2 (índice 2) do CSV
            emails.append(row[2])           # Adiciona o email à lista de emails


    lista_email = []   # Lista para armazenar os emails válidos e inválidos em pares
    num_1 = len(lista_valid_email)     # Número de emails válidos
    num_2 = len(lista_invalid_email)   # Número de emails inválidos
...
list_header = ['Email válido', 'E-mail inválido']
...
app_name = tk.Label(frame_main, text="Verificador de E-mail", height=1, padx=0, relief="flat",
                    anchor="center", font=('Arial', 15, 'bold'), bg=colors['background'], fg=colors['text'])
app_name.place(x=10, y=10)

b_add = tk.Button(frame_main, command=abrir_ficheiro, text="Abrir o ficheiro CSV", height=1,
                  bg=colors['button_bg'], fg=colors['button_fg'], font=('Arial', 10, 'bold'), relief=tk.RAISED)
